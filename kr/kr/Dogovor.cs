...
namespace kr
{
    public partial class Dogovor : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            dataGridView3.Rows[0].Cells[0].Value = "test11";
            dataGridView3.Rows[0].Cells[1].Value = "test12";
            dataGridView3.Rows[0].Cells[2].Value = "test13";
            dataGridView3.Rows[0].Cells[3].Value = "test13";
            dataGridView3.Rows[0].Cells[4].Value = "test13";

            dataGridView3.Rows[1].Cells[0].Value = "test11";
            dataGridView3.Rows[1].Cells[1].Value = "test12";
            dataGridView3.Rows[1].Cells[2].Value = "test13";
            dataGridView3.Rows[1].Cells[3].Value = "test13";
            dataGridView3.Rows[1].Cells[4].Value = "test13";
     ...
     /*  private decimal proc()
        {
            ...
            string s = dataGridView1.CurrentCell.Value.ToString();
            Connection.Open();
            string sqlExpression = "SELECT Календарь.[Основной долг], Календарь.Проценты FROM Календарь  WHERE Календарь.[Номер договора] = '" + s + "'";
            SqlCommand command = new SqlCommand(sqlExpression, Connection);
            SqlDataReader reader = command.ExecuteReader();
            if (reader.Read()) //данные из банка и договора
            {
                var percentmonth = reader.GetValue(1);
                procmonth = Convert.ToDecimal(percentmonth);
                Connection.Close();
                return (procmonth);
            }
            else {
                MessageBox.Show("awda", "pfsa"); Connection.Close(); return (procmonth);
            }
        ...
        private decimal osn()
        {
            ...
            string s = dataGridView1.CurrentCell.Value.ToString();
            Connection.Open();
            string sqlExpression = "SELECT TOP (1) Календарь.[Основной долг], Календарь.Проценты FROM Календарь  WHERE Календарь.[Номер договора] = '" + s + "'";
            SqlCommand command = new SqlCommand(sqlExpression, Connection);
            SqlDataReader reader = command.ExecuteReader();
            if (reader.Read()) //данные из банка и договора
            {
                var osn = reader.GetValue(0);

                osnov = Convert.ToDecimal(osn);
                Connection.Close();
                return (osnov);
            }
            else
            {
                Connection.Close(); return (osnov);
            }
        }
        private void button2_Click(object sender, EventArgs e)
        {
          ...
          //  grafic();
              ...
              double InterestRateYear = Convert.ToDouble(percent); // Процентная ставка, ГОДОВАЯ
              ...
              double procentmonth = SumCredit * (InterestRateYear / 100) / 12; // summ proc
              double osnmonth = Convert.ToDouble(platesh) - procentmonth;//osndolg
              ...
              SqlCommand command = new SqlCommand(sqlExpression, Connection);
               SqlDataReader reader = command.ExecuteReader();
                 ...
                 //клиенты
                  ...
                  ReplaceWordStub("{summa4}", summa, wordDocument);
                  ...
                  double ost = ItogCreditSum - Convert.ToDouble(platesh);
                  double sumproc = ItogCreditSum - Convert.ToDouble(summa);
                  ReplaceWordStub("{percentmonth1}", procentmonth.ToString("N2"), wordDocument);
                  ReplaceWordStub("{ostatok}", ost.ToString("N2"), wordDocument);
                  ReplaceWordStub("{sumproc}", sumproc.ToString("N2"), wordDocument);
                  ReplaceWordStub("{platesh}", platesh, wordDocument);
                  ReplaceWordStub("{percentmonth}", procentmonth.ToString("N2"), wordDocument);
                  ReplaceWordStub("{osndolg}", osnmonth.ToString("N2"), wordDocument);
                  ...
                  ReplaceWordStub("{polnaya3}", ItogCreditSum.ToString("N2"), wordDocument);
                  ReplaceWordStub("{polnaya}", ItogCreditSum.ToString("N2"), wordDocument);
                //  grafic();
                // вставка таблицы
